    ...
    while True:
        ...
        try:
            ...
            if len(signvecs) != len(worker_planes):
                ...
                delta = m.addVar(lb = -1, ub = 1)
                m.setObjective(delta, GRB.MAXIMIZE)
                pointvar = m.addMVar(shape = DIM)
                for i in range(index):
                    if signvecs[i] > 0:
                        m.addConstr(worker_planes[i] @ pointvar >= delta * signvecs[i])
                    else:
                    ...
                    if delta.X > EPSILON:
                        ...
                        with lock:
                            if result < maxsol.value:
                                ...
                                print(f"[Worker {os.getpid()}] New best sol: {maxsol.value}")
            ...
            queue.task_done() # Still need to mark task as done
...
if __name__ == '__main__':
    print("ERMB Algorithm (localsearch) for K-dimensions")
    ...
    for i in range(NUM_DATA):
        ...
        points.append(point) # 'points' is a list of lists here
    ...
    #Sample smaller set of points to use to generate hyperplanes
    num_samples = int(min(len(remaining_points), 100 * math.log2(NUM_DATA)))
    sampled_points = rng.choice(remaining_points, num_samples)
    ...
    with multiprocessing.Pool(processes=16, initializer = ermb_functions.init_globals_kdim, initargs = ([], points, [], {}, K)) as pool:
        partial_generateplanes = partial(ermb_functions.generateplanes_kd_sample, sample = sampled_points)
        nestedplanes = pool.map(partial_generateplanes, sampled_points.tolist())
    ...
    print(f"Final line, {NUM_DATA}, {DIM}, Singleton, {K}, {dominated_size} , ERMB_ls, {round(timetaken,2)}, {sol}")
